...
class Timer:
    ...
    def start(self):
        """Start a new timer"""
        print(datetime.now().strftime("Start time: %m/%d/%Y, %H:%M:%S"))
    ...
    def stop(self):
        ...
        elapsed_time = (time.perf_counter() - self._start_time)/60.0
        self._start_time = None
        print(f"Elapsed time: {elapsed_time:0.2f} minutes")


def displayDataInv(X, example_width=None, figsize=(10, 10)):
    """
    ...
    for i, ax in enumerate(ax_array):
        # Display Image
        h = ax.imshow(X[i].reshape(example_width, example_width, order='C'),
